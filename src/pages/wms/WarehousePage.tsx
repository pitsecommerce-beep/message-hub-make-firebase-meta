...
export default function WarehousePage() {
  ...
  const wmsOrders = demoOrders.filter(o =>
    ['confirmed', 'processing', 'packed', 'shipped'].includes(o.status)
  );
    ...
    <div className="flex h-screen">
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="p-6 pb-0">
          <PageHeader
            title="Almacén (WMS)"
            subtitle={`${wmsOrders.length} pedidos en proceso`}
          />

          {/* Stats */}
          <div className="grid grid-cols-4 gap-3 mb-5">
            {WMS_STAGES.map(stage => {
              const count = demoOrders.filter(o => o.status === stage.status).length;
              return (
                <button
                  key={stage.status}
                  ...
                  className={classNames(
                    ...
                    activeTab === stage.status ? 'ring-2 ring-primary-300 shadow-md' : 'hover:shadow-md'
                  )}
                >
                  <div className={`w-9 h-9 rounded-lg ${stage.color} flex items-center justify-center mb-2`}>
                    {stage.icon}
                  </div>
            ...
            {tabOrders.map(order => (
              <div
                ...
                className={classNames(
                  ...
                  selectedOrder?.id === order.id ? 'ring-2 ring-primary-300' : ''
                )}
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-primary-600">{order.orderNumber}</p>
                      <p className="text-xs text-surface-400">{order.contact?.name} - {order.items.length} items</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className={getOrderStatusColor(order.status)}>{getOrderStatusLabel(order.status)}</span>
                    <span className="text-sm font-medium text-surface-700">{formatCurrency(order.total)}</span>
                  </div>
                </div>

                <div className="mt-3 flex items-center gap-2 flex-wrap">
                  {order.items.map(item => (
                    <span key={item.id} className="badge-neutral text-xs">
        ...
        <div className="w-96 border-l border-surface-200 bg-white overflow-y-auto flex-shrink-0">
          <div className="p-5">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setSelectedOrder(null)} className="p-1 rounded hover:bg-surface-100 text-surface-400">
                <X size={16} />
              </button>
            </div>
            ...
            <div className="mb-5">
              <h4 className="text-xs font-medium text-surface-400 mb-2">Cliente</h4>
              <div className="flex items-center gap-2 p-2 rounded-lg bg-surface-50">
                ...
                <span className="text-sm font-medium text-surface-800">{selectedOrder.contact?.name}</span>
              </div>
            </div>

            <div className="mb-5">
              <h4 className="text-xs font-medium text-surface-400 mb-2">Productos a preparar</h4>
              <div className="space-y-2">
                {selectedOrder.items.map(item => (
                  <div key={item.id} className="flex items-center gap-3 p-3 rounded-lg border border-surface-200">
                    <div className="w-8 h-8 rounded bg-surface-100 flex items-center justify-center text-surface-500">
                      <Box size={16} />
                    </div>
                    <div className="flex-1">
                      <p className="text-sm font-medium text-surface-800">{item.name}</p>
                      <p className="text-xs text-surface-400">{item.sku || 'Sin SKU'}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-sm font-bold text-surface-800">x{item.quantity}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {selectedOrder.shippingAddress && (
              <div className="mb-5">
                <h4 className="text-xs font-medium text-surface-400 mb-1 flex items-center gap-1">
                  <MapPin size={12} /> Enviar a
                </h4>
                <p className="text-sm text-surface-700 p-2 rounded-lg bg-surface-50">{selectedOrder.shippingAddress}</p>
              </div>
            )}
            ...
            {selectedOrder.trackingNumber && (
              <div className="mb-5">
                ...
                <p className="text-sm font-medium text-primary-600">{selectedOrder.trackingNumber}</p>
              </div>
            )}

            {selectedOrder.notes && (
              <div className="mb-5 p-3 rounded-lg bg-amber-50 border border-amber-100">
                <p className="text-xs font-medium text-amber-700 mb-0.5">Notas especiales</p>
            ...
            {getNextStatus(selectedOrder.status) && (
              ...
              >
                Avanzar a {getOrderStatusLabel(getNextStatus(selectedOrder.status)!)}
                <ArrowRight size={16} />
              </button>
            )}
